...
static QState AO_System_initial(AO_System_t * const me, void const * const par) {
    ...
    QTimeEvt_armX(&me->led_timer, 
                  TIME_EVENT_LED_MS,      // 首次触发时间
                  TIME_EVENT_LED_MS);     // 周期
...
static QState AO_System_normal(AO_System_t * const me, QEvt const * const e) {
    ...
    switch (e->sig) {
        case Q_ENTRY_SIG: {
            printf("[AO-SYSTEM] Entering NORMAL state\n");
        ...
        case ERROR_SIG: {
            ...
            me->error_count++;
            printf("[AO-SYSTEM] Error received: code=%d, count=%lu\n", 
        ...
        case ESTOP_SIG: {
            printf("[AO-SYSTEM] Emergency stop triggered!\n");
            status = Q_TRAN(&AO_System_emergency_stop);
            break;
        }
        
        case CMD_FLASH_SAVE_SIG: {
            printf("[AO-SYSTEM] Saving parameters to Flash...\n");
        ...
        case CMD_FLASH_LOAD_SIG: {
            printf("[AO-SYSTEM] Loading parameters from Flash...\n");
            param_manager_load();
            status = Q_HANDLED();
            break;
        }
...
static QState AO_System_error(AO_System_t * const me, QEvt const * const e) {
    ...
    switch (e->sig) {
        case Q_ENTRY_SIG: {
            printf("[AO-SYSTEM] Entering ERROR state\n");
        ...
        case INIT_COMPLETE_SIG: {
            // 收到重新初始化完成信号，返回正常状态
            printf("[AO-SYSTEM] Recovery, returning to NORMAL\n");
...
static QState AO_System_emergency_stop(AO_System_t * const me, QEvt const * const e) {
    ...
    switch (e->sig) {
        case Q_ENTRY_SIG: {
            printf("[AO-SYSTEM] EMERGENCY STOP!\n");
