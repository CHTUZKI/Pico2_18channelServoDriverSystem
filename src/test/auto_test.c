...
void auto_test_on_motion_complete(void) {
    ...
    switch (test_ctx.state) {
        case TEST_STATE_TO_0:
            ...
            printf("[AUTO-TEST] Cycle %lu/%lu - Moving to 180deg\n", 
        ...
        case TEST_STATE_TO_180:
            ...
            if (test_ctx.cycle_count < test_ctx.total_cycles) {
                ...
                printf("[AUTO-TEST] Cycle %lu/%lu - Moving to 0deg\n", 
                       test_ctx.cycle_count, test_ctx.total_cycles);
                test_send_motion_command(0.0f, TEST_DURATION_MS);
            } else {
                // 测试完成，回到90度安全位置
                test_ctx.state = TEST_STATE_COMPLETE;
                printf("[AUTO-TEST] Returning to 90deg (safe position)\n");
        ...
        case TEST_STATE_COMPLETE:
            ...
            printf("\n========================================\n");
            printf("[AUTO-TEST] Test completed!\n");
...
void auto_test_start(void) {
    printf("\n========================================\n");
    printf("[AUTO-TEST] Cycle test mode\n");
    printf("[AUTO-TEST] Total cycles: %lu\n", TEST_CYCLES);
    printf("[AUTO-TEST] Sequence: 0deg <-> 180deg\n");
    printf("[AUTO-TEST] Duration: %ums per move\n", TEST_DURATION_MS);
    printf("========================================\n\n");
    ...
    // 开始第一次运动：移动到0度
    printf("[AUTO-TEST] Cycle 1/%lu - Moving to 0deg\n", TEST_CYCLES);
    test_send_motion_command(0.0f, TEST_DURATION_MS);
}
